...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function DeepSeekQuotaBadge(props) {
			...
			// ---- 拖拽：window 级常驻监听，不依赖 pointer capture ----
			// 之前依赖 setPointerCapture + 元素上的 pointerup：只要 pointerup
			// 丢失（窗口外松手、切走焦点、捕获失败被吞），dragRef 就永久残留，
			// 拖拽卡死（刷新才恢复）。改为 window 级监听：任何位置松手都能
			// 收尾，blur/页面隐藏兜底强制结束，每次按下也先清理残留。
			// finishDrag：幂等收尾——夹取当前位置、写回 userPos 并持久化。
			const finishDrag = () => {
			...
			// 常驻 window 监听：拖拽期间处理 move/up/cancel，平时 no-op。
			// 引用的 ref 与 setter 全部稳定，effect 空依赖闭包无陈旧问题。
			useEffect(() => {
			...
			const onPointerDown = (e) => {
				...
				// 上一次拖拽可能未正常收尾（pointerup 丢失）：先清理残留。
				if (dragRef.current) finishDrag();
				const el = cardRef.current;
				if (!el) return;
				const parent = el.offsetParent || el.parentElement;
